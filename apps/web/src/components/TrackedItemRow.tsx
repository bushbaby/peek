...
export function TrackedItemRow({
  ...
  return (
    <tr className={`border-b border-line-subtle hover:bg-ghost ${item.is_paused ? 'opacity-60' : ''}`}>
      ...
      <td className="px-4 py-3">
        ...
        {item.last_error_message && item.last_status === 'error' && (
          <p className="mt-0.5 text-xs text-red-600 truncate max-w-40 dark:text-red-400" title={item.last_error_message}>
